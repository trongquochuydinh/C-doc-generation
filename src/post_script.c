#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
#include "post_script.h"
#include "parser.h"

#define PI 3.14159265358979323846
#define EPSILON 0.001
#define Y_THRESHOLD 10.0  /* Threshold to skip large y-values for asymptotes */

void generate_postscript(const char *outfile, const char *func, double x_min, double x_max, double y_min, double y_max, int calc_x_range, int calc_y_range) {
    FILE *ps_file = fopen(outfile, "w");
    if (ps_file == NULL) {
        fprintf(stderr, "Error opening file for writing: %s\n", outfile);
        exit(1);
    }

    fprintf(ps_file, "%%!PS-Adobe-2.0\n");
    fprintf(ps_file, "%%%%BoundingBox: 0 0 500 500\n");

    double step = 0.001;
    double x, y;
    Node* expression_tree = parse_expression(&func);

    if ((strstr(func, "ln(x)") || strstr(func, "log(x)")) && calc_x_range) {
        x_min = 0.1;
        x_max = 10;
    }

    if (calc_x_range) {
        x_min = -10;
        x_max = 10;
    }
    if (calc_y_range) {
        y_min = -10;
        y_max = 10;
    } else {
        y_min = INFINITY;
        y_max = -INFINITY;
        for (x = x_min; x <= x_max; x += step) {
            y = evaluate(expression_tree, x);
            if (!isnan(y)) {
                if (y < y_min) y_min = y;
                if (y > y_max) y_max = y;
            }
        }
    }

    double x_range = x_max - x_min;
    double y_range = y_max - y_min;
    double x_scale = 300.0 / x_range;
    double y_scale = 300.0 / y_range;
    double x_offset = 250 - (x_range * x_scale) / 2;
    double y_offset = 250 - (y_range * y_scale) / 2;

    /* Draw grid lines */
    fprintf(ps_file, "newpath\n");
    fprintf(ps_file, "0.8 0.8 0.8 setrgbcolor\n");  /* Light gray for grid lines */

    int i;
    for (i = 100; i <= 400; i += 30) {
        /* Vertical grid lines */
        fprintf(ps_file, "%d 100 moveto\n", i);
        fprintf(ps_file, "%d 400 lineto\n", i);
        fprintf(ps_file, "stroke\n");

        /* Horizontal grid lines */
        fprintf(ps_file, "100 %d moveto\n", i);
        fprintf(ps_file, "400 %d lineto\n", i);
        fprintf(ps_file, "stroke\n");
    }

    /* Draw graph */
    fprintf(ps_file, "newpath\n");
    fprintf(ps_file, "1 0 0 setrgbcolor\n");
    int start_new_line = 1;
    for (x = x_min; x <= x_max; x += step) {
        y = evaluate(expression_tree, x);
        if (isnan(y)) {
            start_new_line = 1;
            continue;
        }
        double ps_x = x_offset + (x - x_min) * x_scale;
        double ps_y = y_offset + (y - y_min) * y_scale;
        if (ps_x < 100 || ps_x > 400 || ps_y < 100 || ps_y > 400) {
            start_new_line = 1;
            continue;
        }
        if (start_new_line) {
            fprintf(ps_file, "%lf %lf moveto\n", ps_x, ps_y);
            start_new_line = 0;
        } else {
            fprintf(ps_file, "%lf %lf lineto\n", ps_x, ps_y);
        }
    }
    fprintf(ps_file, "stroke\n");

    /* Draw bounding box and axis labels */
    fprintf(ps_file, "newpath\n");
    fprintf(ps_file, "0 0 0 setrgbcolor\n");
    fprintf(ps_file, "100 100 moveto\n");
    fprintf(ps_file, "400 100 lineto\n");
    fprintf(ps_file, "400 400 lineto\n");
    fprintf(ps_file, "100 400 lineto\n");
    fprintf(ps_file, "100 100 lineto\n");
    fprintf(ps_file, "stroke\n");

    fprintf(ps_file, "/Courier findfont 9 scalefont setfont\n");
    fprintf(ps_file, "250 60 moveto\n");
    fprintf(ps_file, "(x) show\n");

    fprintf(ps_file, "30 250 moveto\n");
    fprintf(ps_file, "90 rotate\n");
    fprintf(ps_file, "(f(x)) show\n");
    fprintf(ps_file, "-90 rotate\n");

    for (i = 100; i <= 400; i += 30) {
        fprintf(ps_file, "%d 90 moveto\n", i);
        fprintf(ps_file, "%d 110 lineto\n", i);
        fprintf(ps_file, "stroke\n");
        fprintf(ps_file, "%d 80 moveto\n", i - 10);
        fprintf(ps_file, "(%0.1f) show\n", x_min + (i - 100) * x_range / 300.0);

        fprintf(ps_file, "90 %d moveto\n", i);
        fprintf(ps_file, "110 %d lineto\n", i);
        fprintf(ps_file, "stroke\n");
        fprintf(ps_file, "50 %d moveto\n", i - 5);
        fprintf(ps_file, "(%0.1f) show\n", y_min + (i - 100) * y_range / 300.0);
    }

    free_tree(expression_tree);
    fclose(ps_file);
}
